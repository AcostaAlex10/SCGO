...
| **C-10** | P2 | **Dependencias del front.** Empezado en la rama `claude/ingenieria-software-nube-2ws8jg` (PR #20): al borrar dos primitivos muertos salieron `react-day-picker`, `date-fns` y `react-resizable-panels`, y el paquete bajó de 1.032,58 kB a 952,48 kB (gzip: 293,21 → 269,86). Quedan MUI, Radix y shadcn conviviendo. Importa por RNF01: la carga en obra con señal móvil. Además, dos actualizaciones que no se pueden subir sueltas: **Vite 7 junto con `@vitejs/plugin-react` 6**, que importa `vite/internal` (cerrado el PR #16), y **React 19 con `react`, `react-dom` y sus tipos juntos**, porque subir solo `react-dom` deja la aplicación en blanco (cerrado el PR #17; el #28 lo intentó al revés, subiendo `react` sin `react-dom`, y se cerró igual). En la rama `claude/preparar-dependabot` (PR #33) salieron `motion` y `react-responsive-masonry`, que no usaba nadie, y el código quedó listo para **TypeScript 7** y **recharts 3**, que entraron después en el #27 y el #29. El Dashboard se dibuja igual con recharts 3; queda anotado que recharts 3 toma como peer el `react-is` 19 que trae MUI, con React 18, sin efecto visible: se resuelve con React 19. | M | Codex |
| **C-11** | P2 | **Código muerto.** `JsonProyectoRepository` no lo usa nadie en ejecución: es el prototipo con archivo JSON anterior a la base. En el front, la rama `claude/ingenieria-software-nube-2ws8jg` (PR #20) borró `resizable.tsx` y `calendar.tsx`; `chart.tsx` se borró en el PR #33 porque recharts 3 le rompía los tipos; siguen sin usarse `carousel` y `drawer`. | S | Codex |
...
7. ~~**B-03**~~ (migraciones, PR #23), ~~**B-04**~~ (monitoreo, PR #24) y ~~**B-05**~~ (health con base, PR #21).
...
| 2026-09-19 | B-03 hecho (PR #23): `schema_migrations` y un comando que aplica lo pendiente. Por decisión del equipo no corre en el deploy: se corre a mano después. `intento_login` deja de crearse en tiempo de ejecución. |
